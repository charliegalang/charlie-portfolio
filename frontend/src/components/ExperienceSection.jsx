...
const ExperienceSection = ({ isEditMode, data, onChange, mode = 'light' }) => {
  ...
  return (
    <section
      className="py-16 sm:py-32 md:py-40 px-4 sm:px-12 md:px-20 lg:px-32 transition-colors duration-500 relative overflow-hidden"
    ...
    >
      ...
      {isEditMode && (
          <div className="absolute top-4 right-4 sm:right-12 md:right-32 flex flex-wrap items-center gap-3 z-50">
            <ColorPickerField
              label="Section"
      ...
      <div className="max-w-screen-2xl mx-auto relative">
        <div className="relative group/header mb-12 sm:mb-16">
          <div
            role="button"
            tabIndex={0}
            onClick={() => !expData.isLocked && setIsExpanded(!isExpanded)}
            onKeyDown={(e) => e.key === 'Enter' && !expData.isLocked && setIsExpanded(!isExpanded)}
            className={`w-full flex flex-col sm:flex-row items-center sm:justify-between gap-6 sm:gap-8 p-6 sm:p-10 rounded-2xl sm:rounded-[2rem] border-2 border-theme/10 hover:border-theme/30 transition-all outline-none shadow-2xl ${expData.isLocked ? 'cursor-default' : 'cursor-pointer'}`}
          ...
          >
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 text-center sm:text-left w-full sm:w-auto">
              <div className="p-4 sm:p-6 rounded-2xl sm:rounded-3xl relative group/icon bg-theme-icon/5 border-2 border-theme-icon/10">
                <div className="relative group/iconEdit">
                  <IconComponent
                    name={headerIconName}
                    size={isEditMode ? 32 : 40}
              ...
              <div className="space-y-1">
                <RichTextEditor
                  ...
                  className="text-2xl sm:text-4xl md:text-5xl font-black text-theme-primary tracking-tighter"
                  placeholder="Professional Experience"
                />
                <p className="text-xs sm:text-base text-theme-secondary opacity-60 font-medium">
            ...
            {!expData.isLocked && (
              <motion.div
                ...
                className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-theme-secondary group-hover:bg-theme-icon/10 group-hover:text-theme-icon transition-all"
              >
          ...
          {isEditMode && (
            <div className="absolute -top-4 right-0 sm:top-4 sm:right-16 flex items-center gap-2 z-50">
              <button
                onClick={(e) => { e.stopPropagation(); toggleLock(); }}
                className={`p-1.5 sm:p-2 rounded-xl border transition-all flex items-center gap-1.5 opacity-100 sm:opacity-0 sm:group-hover/header:opacity-100 ${expData.isLocked ? 'bg-orange-500/20 border-orange-500/50 text-orange-500' : 'bg-blue-500/20 border-blue-500/50 text-blue-500'}`}
              >
                {expData.isLocked ? <Lock size={12} /> : <LockOpen size={12} />}
                <span className="text-[8px] sm:text-[10px] font-black uppercase">{expData.isLocked ? 'Locked' : 'Manual'}</span>
              </button>
              <CrudControls
                onAdd={addWork}
          ...
          {isExpanded && (
            ...
            >
              <div className="grid lg:grid-cols-12 gap-8 sm:gap-16">
                ...
                <div className="lg:col-span-4 space-y-8 sm:y-12 order-2 lg:order-1">
                  <div
                    className="bg-theme-primary rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 border border-theme/10 relative group/summary shadow-xl"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-4 sm:mb-6">
                      <div className="w-6 sm:w-8 h-1 bg-theme-line rounded-full" />
                      <RichTextEditor
                        value={parseMultiMode(expData.professionalSummaryTitle)[mode]}
                        onSave={(val) => updateField('professionalSummaryTitle', val)}
                        isEditMode={isEditMode}
                        className="text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-theme-primary/50"
                    ...
                    <RichTextEditor
                      ...
                      isEditMode={isEditMode}
                      className="text-base sm:text-xl text-theme-primary font-medium leading-relaxed"
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    <div className="flex items-center gap-4 px-2">
                      <GraduationCap className="text-theme-line" size={20} />
                      <RichTextEditor
                        value={parseMultiMode(expData.educationTitle)[mode]}
                        onSave={(val) => updateField('educationTitle', val)}
                        isEditMode={isEditMode}
                        className="text-xl sm:text-2xl font-black text-theme-primary tracking-tight"
                    ...
                    {isEditMode && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3 sm:py-4 border-2 border-dashed border-theme/20 rounded-xl sm:rounded-2xl text-theme-primary/40 hover:border-theme-line hover:text-theme-line transition-all font-bold text-xs sm:text-sm"
                      >
                    ...
                    <div className="space-y-4 sm:space-y-6">
                      {(expData.educationList || []).map((edu, index) => {
                        ...
                        return (
                          <motion.div
                            key={edu.id}
                            className="bg-theme-primary rounded-xl sm:rounded-2xl p-5 sm:p-6 border border-theme/10 relative group/edu shadow-md"
                            style={{ backgroundColor: eduBg[mode] !== 'transparent' ? eduBg[mode] : '' }}
                          >
                            {isEditMode && (
                              <div className="absolute top-2 right-2 z-50 opacity-100 sm:opacity-0 sm:group-hover/edu:opacity-100 transition-opacity">
                                <CrudControls
                            ...
                            <RichTextEditor
                              ...
                              isEditMode={isEditMode}
                              className="text-sm sm:text-base font-bold text-theme-primary"
                              placeholder="Degree"
                            />
                            <RichTextEditor
                              ...
                              isEditMode={isEditMode}
                              className="text-xs sm:text-sm text-theme-secondary opacity-60 font-medium mt-1"
                            ...
                            <RichTextEditor
                              ...
                              isEditMode={isEditMode}
                              className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-theme-line mt-2 sm:mt-3"
                              placeholder="Period"
                            />
                          </motion.div>
                        );
                ...
                <div className="lg:col-span-8 space-y-8 sm:space-y-12 order-1 lg:order-2">
                  <div className="flex items-center gap-4 px-2">
                    <Briefcase className="text-theme-line" size={20} />
                    <RichTextEditor
                  ...
                  <div className="space-y-10 sm:space-y-16">
                    {expData.work.map((job, index) => {
                      ...
                      return (
                        <motion.div
                          key={job.id}
                          className="relative pl-8 sm:pl-16 border-l-2 border-theme/10 group/job pb-4"
                        >
                          ...
                          {isEditMode && (
                            <div className="absolute -top-10 sm:-top-8 right-0 z-50 opacity-100 sm:opacity-0 sm:group-hover/job:opacity-100 transition-opacity">
                              <CrudControls
                          ...
                          <div
                            className="bg-theme-primary rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 border border-theme/10 hover:border-theme/30 hover:shadow-2xl transition-all relative"
                          ...
                          >
                            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 mb-6 sm:mb-10">
                              <div className="space-y-1 sm:space-y-2">
                                <RichTextEditor
                                  ...
                                  isEditMode={isEditMode}
                                  className="text-xl sm:text-3xl font-black text-theme-primary tracking-tight"
                                ...
                                <RichTextEditor
                                  ...
                                  isEditMode={isEditMode}
                                  className="text-base sm:text-lg font-bold text-theme-line"
                                  placeholder="Company"
                                />
                              </div>
                              <div className="text-left sm:text-right space-y-2 sm:space-y-3">
                                <RichTextEditor
                                  ...
                                  isEditMode={isEditMode}
                                  className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-theme-primary/40"
                                  placeholder="Dates"
                                />
                                <RichTextEditor
                                  ...
                                  isEditMode={isEditMode}
                                  className="inline-block px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest bg-theme-icon/10 text-theme-icon"
                                  placeholder="Type"
                                />
                              </div>
                            </div>
                            ...
                            <ul className="space-y-4 sm:space-y-6">
                              {job.responsibilities.map((resp) => (
                                <li key={resp.id} className="flex gap-4 sm:gap-6 group/resp">
                                  <div className="mt-2 w-1.5 h-1.5 rounded-full bg-theme-line/40 shrink-0" />
                                  <div className="flex-1 relative">
                                    <RichTextEditor
                                      value={parseMultiMode(resp.text)[mode]}
                                      onSave={(val) => updateResponsibility(job.id, resp.id, val)}
                                      isEditMode={isEditMode}
                                      className="text-sm sm:text-lg text-theme-secondary font-medium leading-relaxed"
                                      placeholder="Key responsibility..."
                                    />
                                    {isEditMode && (
                                      <div className="absolute -right-10 sm:-right-12 top-0 opacity-100 sm:opacity-0 sm:group-hover/resp:opacity-100 transition-opacity">
                                        <CrudControls
